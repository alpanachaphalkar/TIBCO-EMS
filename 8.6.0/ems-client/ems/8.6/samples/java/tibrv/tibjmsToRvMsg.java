 ...
 * 
 * $Id: tibjmsToRvMsg.java 90180 2016-12-13 23:00:37Z $
 * 
 */
  ...
  *
  * Usage: java tibjmsToRvMsg [options]
  *
  *     where options are:
  *
...
public class tibjmsToRvMsg
{
    ...
    class RvCallback implements TibrvMsgCallback
    {
        public void onMsg(TibrvListener listener, TibrvMsg msg)
        {
            try
            {
                ...
                /* now add a submessage */
                TibrvMsg submsg = new TibrvMsg();
                submsg.add("rv_sub_field","this is a field in RV submessage");

                response.add("submsg", submsg);
     ...
     */
    public tibjmsToRvMsg(String[] args)
    {
        parseArgs(args);
        ...
        try
        {
            ...
            /* now let's try to use extensions */
            MapMessage submessage = publish_session.createMapMessage();
            submessage.setString("sub_field","this is a field in a submessage");

            /* now let's check we do get the standard
             * MessageFormatException if we try this:
             */
            try
            {
                sendMessage.setObject("submessage",submessage);
                System.err.println("**Error** failed to receive exception when adding message into message");
            }
            catch (MessageFormatException e)
            {
                System.err.println("OK - received exception when tried to add message into message:");
                System.err.println("     "+e.getMessage());
            }

            /*
             * now let's enable message extensions and try same
             */
            sendMessage.setBooleanProperty("JMS_TIBCO_MSG_EXT",true);

            try
            {
                sendMessage.setObject("submessage",submessage);
                System.err.println("OK - now successfully added a message as a field of another message");
            }
            catch (MessageFormatException e)
            {
                System.err.println("**Error** still received exception when tried to add message into message:");
                e.printStackTrace();
            }

            /* now also try to add an array */
            short[] array = new short[5];
            for (int i=0; i<array.length; i++)
                array[i] = (short)i;

            sendMessage.setObject("short_array",array);
            ...
            else
            {
                ...
                /* we expect a message with extensions enabled and
                 * which has a submessage named submsg
                 */
                if (reply instanceof MapMessage)
                {
                    Object object = ((MapMessage)reply).getObject("submsg");

                    if (object == null)
                    {
                        System.err.println("**Error** submessage not found");
                    }
                    else
                    {
                        if (object instanceof MapMessage)
                            System.err.println("Found field 'submsg' which is MapMessage");
                        else
                            System.err.println("**Error** field 'submsg' is of wrong type "+object.getClass().getName());
                    }
                }
    ...
    {
        tibjmsToRvMsg t = new tibjmsToRvMsg(args);
    }
