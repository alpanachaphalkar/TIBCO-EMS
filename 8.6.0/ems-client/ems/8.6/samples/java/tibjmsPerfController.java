 ...
 * 
 * $Id: tibjmsPerfController.java 129199 2020-10-07 22:55:14Z $
 * 
 */

/*
 * The tibjmsPerfController class publishes instructions to the
 * tibjmsPerfWorker class and invokes the performance message
 * producer. At the end of the run the tibjmsPerfWorker class
 ...
 *  
 * Usage:  java tibjmsPerfController [options]
 *
 *  where options are:
 *
...
public class tibjmsPerfController
{
    /*
     * constants
     */
     ...
     */
    public tibjmsPerfController(String[] args)
    {
        parseArgs(args);
    ...
    private void printStartBanner()
    {
        System.err.println();
        System.err.println("------------------------------------------------------------------------");
        System.err.println("tibjmsPerfController program");
        System.err.println("------------------------------------------------------------------------");
     ...
     * instructions to the worker control program.
     */
    ...
    /**
     * Send instructions to the worker program.
     */
    private void sendInstructions() throws JMSException
    {
        ...
        // wait for all worker programs to respond
        int numReceivers = serverAdmin.getNumberOfReceivers(destination);
        while (numReceivers-- > 0)
        {
            System.err.println("Waiting for response from " + (numReceivers + 1) + " worker program(s)");
    ...
    private void printPerformance() throws JMSException
    {
        ...
        while (numReceivers-- > 0)
        {
            System.err.println("Waiting for performance results from " +
                (numReceivers + 1) + " worker program(s)");
    ...
    {
        System.err.println();
        System.err.println("Usage: java tibjmsPerfController [options]");
        System.err.println();
    ...
    {
        tibjmsPerfController t = new tibjmsPerfController(args);
    }
}
