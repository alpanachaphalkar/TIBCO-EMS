 ...
 * 
 * $Id: tibemsBrowser.c 108237 2019-03-18 21:34:01Z $
 * 
 */
...
void usage() 
{
    ...
    baseUtils_print(" -queue    <queue-name> - queue name, default is \"queue.sample.browser\"\n");
...
void run() 
{
    ...
    if (!queueName) {
        baseUtils_print("***Error: must specify queue name\n");
        usage();
    }
    
    baseUtils_print("Browser sample.\n");
    baseUtils_print("Using server:   %s\n", serverUrl ? serverUrl : "(null)");
    baseUtils_print("Browsing queue: %s\n",queueName);
    ...
    baseUtils_print("Draining the queue %s\n",queueName);
    ...
    baseUtils_print("Drained %d messages from the queue.\n",drainc);
     ...
     */
    baseUtils_print("Sending 5 messages into queue.\n");
    for (i=0; i<5; i++) 
    {
        messagec++;
     ...
     */
    baseUtils_print("--- Browsing the queue.\n");
    ...
    while(1) 
    {
        ...
        baseUtils_print("Browsed message: number=%d\n",msg_num);
    ...
    baseUtils_print("--- No more messages in the queue.\n");
            
    /*
     * send 5 more messages into queue
     */
    baseUtils_print("Sending 5 more messages into queue.\n");
    for (i=0; i<5; i++) 
    {
        messagec++;
    ...
    while(1) 
    {
        ...
        baseUtils_print("Waiting for messages to arrive, count=%d\n",attemptc);

        tibems_Sleep(1000);

        if (attemptc > 30)
        {
            baseUtils_print("Still no messages in the queue after %d seconds.\n",attemptc);
            closeAll();
            exit(0);
        }
    }
     ...
     */
    baseUtils_print("Found more messages. Continue browsing.\n");
    while(msg) 
    {
        /* get the msg_num Integer Property */
        status = tibemsMsg_GetIntProperty(msg,"msg_num",&msg_num);
