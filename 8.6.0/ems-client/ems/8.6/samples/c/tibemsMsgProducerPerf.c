 ...
 * 
 * $Id: tibemsMsgProducerPerf.c 108237 2019-03-18 21:34:01Z $
 * 
 */
...
void usage() 
{
    ...
    baseUtils_print(" -size         <bytes>       Message payload size in bytes. Default is 0.\n");
    baseUtils_print(" -delivery     <mode>        Delivery mode: RELIABLE, NON_PERSISTENT, PERSISTENT\n");
    baseUtils_print("                             Default is NON_PERSISTENT.\n");
    ...
    baseUtils_print(" -threads      <num threads> Number of producer threads.\n");
    baseUtils_print(" -connections  <num conns>   Number of connections.\n");
    baseUtils_print(" -rate         <msg/sec>     Message rate for producer threads.\n");
    baseUtils_print(" -payload      <filename>    File containing message payload.\n");
    baseUtils_print(" -uniquedests                Each producer uses a unique destination.\n");
    baseUtils_print(" -compression                Enable message compression.\n");
    baseUtils_print(" -async                      Send messages asynchronously.\n");
    exit(0);
}
...
void parseArgs(int argc, char** argv) 
{
    ...
    while (i < argc)
    {
        ...
        else if (strcmp(argv[i],"-size")==0)
        {
            ...
            if (msgSize < 0)
            {
                baseUtils_print("Invalid value for -size: %s\n", argv[i+1]);
                usage();
            }
            i += 2;
        }
        else if (strcmp(argv[i],"-delivery")==0)
        {
            ...
            {
                baseUtils_print("Invalid value for -delivery: %s\n", argv[i+1]);
                usage();
            }
            i += 2;
        }
...
void createMessage(tibemsMsg* msg)
{
    ...
    if (payloadFile)
    {
        ...
        if (file == NULL)
        {
            baseUtils_print("Error opening %s: %s\n", payloadFile, strerror(errno));
        }
        else
        {
...
THREAD_RETVAL run(void* a)
{
    ...
    if (async)
    {
        ...
        if (!clArgs)
        {
            baseUtils_print("Unable to create completion listener arguments.\n");
            exit(1);
        }
    }
...
int main(int argc, char** argv)
{
    ...
    baseUtils_print("------------------------------------------------------------------------\n");
    baseUtils_print("tibemsMsgProducerPerf\n");
    baseUtils_print("------------------------------------------------------------------------\n");
    baseUtils_print("Server....................... %s\n", serverUrl != NULL ? serverUrl : "localhost");
    baseUtils_print("User......................... %s\n", username != NULL ? username : "(NULL)");
    baseUtils_print("Destination.................. %s\n", destination != NULL ? destination : "(NULL)");
    if (payloadFile)
        baseUtils_print("Message Size................. %s\n", payloadFile);
    else
        baseUtils_print("Message Size................. %d\n", msgSize);
    baseUtils_print("Producer Threads............. %d\n", threads);
    baseUtils_print("Producer Connections......... %d\n", connections);
    baseUtils_print("Delivery Mode................ %s\n", deliveryModeToString(deliveryMode));
    baseUtils_print("Compression.................. %s\n", useCompression == TIBEMS_TRUE ? "TRUE" : "FALSE");
    baseUtils_print("Asynchronous Sending......... %s\n", async == TIBEMS_TRUE ? "TRUE" : "FALSE");
    if (txnSize > 0)
    {
    ...
    if ( stop > start )
    {
        ...
        { 
           ...
           { 
               ...
               baseUtils_print("%d completion listeners took %d milliseconds, performance is %.0f calls/second\n",
                   total, (tibems_int)(completionTime-start), rate);
           } 
           else 
           { 
               baseUtils_print("interval too short to calculate a completion listener rate\n"); 
           } 
        }
    }
    else
